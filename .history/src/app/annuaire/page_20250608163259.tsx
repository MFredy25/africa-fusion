...
export default function Annuaire() {
  ...
  /* ─── Thème & responsive colonnes ─── */
  useEffect(() => {
    const mediaDark = window.matchMedia('(prefers-color-scheme: dark)');
    const setDark = () => setIsDark(mediaDark.matches);
    setDark();
    mediaDark.addEventListener('change', setDark);

    const setCols = () => setColumns(window.innerWidth < 768 ? 3 : 5);
    setCols();
    window.addEventListener('resize', setCols);

    return () => {
      mediaDark.removeEventListener('change', setDark);
  ...
  return (
    ...
    >
      {/* ─── Header réutilisé depuis HubHeader ─── */}
      <header
        style={{
          display: 'flex',
      ...
      >
        ...
        <nav style={{ display: 'flex', gap: '1.5rem', color: colors.text }}>
